...
func (r *Reader) Page(num int) Page {
	...
	// unblink patch: a malformed or cyclic page tree must terminate the search.
	// Upstream loops forever when a Pages node's kids don't resolve (the outer
	// loop re-evaluates the same node) or when kids form a reference cycle, so
	// (a) scanning all kids without descending ends the search, and (b) the
	// number of descents is bounded (a page tree deeper than this is hostile).
	const maxPageTreeDepth = 1 << 12
	for depth := 0; page.Key("Type").Name() == "Pages" && depth < maxPageTreeDepth; depth++ {
		...
		if !descended {
			return Page{} // exhausted this node's kids without finding the page
		}
	}
	return Page{}
}
