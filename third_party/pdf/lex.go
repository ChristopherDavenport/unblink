...
type buffer struct {
	...
	useAES      bool
	objptr      objptr
	synthetic   int // consecutive fabricated newlines from an exhausted reader (unblink patch)
}

// maxSyntheticNewlines bounds how many bytes readByte may fabricate after the
// underlying reader stops producing data. Upstream fabricates '\n' forever,
// which turns a malformed xref into an infinite in-memory loop (fuzz-found;
// see unblink docs/decisions/0001). Legitimate parses need at most a few
// synthetic newlines to terminate a token at true EOF.
const maxSyntheticNewlines = 1 << 12
...
func (b *buffer) readByte() byte {
	if b.pos >= len(b.buf) {
		...
		if b.pos >= len(b.buf) {
			b.synthetic++
			if b.synthetic > maxSyntheticNewlines {
				panic("pdf: reader exhausted: malformed input (synthetic newline bound exceeded)")
			}
			return '\n'
		}
	}
